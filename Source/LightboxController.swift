...
public protocol LightboxControllerDownloadDelegate: AnyObject {
    func lightboxController(_ controller: LightboxController, page: Int, didPressDownloadButton deleteButton: UIButton)
}

public protocol LightboxControllerProtocol {
    var didPressDownloadButton: ((_ controller: LightboxController, _ page: Int, _ downloadButton: UIButton) -> Void)? { get set }
    var willDismiss: ((_ controller: LightboxController) -> Void)? { get set }
    var didTouch: ((_ controller: LightboxController, _ image: LightboxImage, _ index: Int) -> Void)? { get set }
    var didMoveToPage: ((_ controller: LightboxController, _ page: Int) -> Void)? { get set }
}

open class LightboxControllerViewModel: LightboxControllerProtocol {
    public var didPressDownloadButton: ((_ controller: LightboxController, _ page: Int, _ downloadButton: UIButton) -> Void)?
    public var willDismiss: ((_ controller: LightboxController) -> Void)?
    public var didTouch: ((_ controller: LightboxController, _ image: LightboxImage, _ index: Int) -> Void)?
    public var didMoveToPage: ((_ controller: LightboxController, _ page: Int) -> Void)?
}
...
open class LightboxController: UIViewController {
    ...
    open lazy var headerView: HeaderView = { [unowned self] in
        let view = HeaderView()
...
//            pageDelegate?.lightboxController(self, didMoveToPage: currentPage)
            viewModel.didMoveToPage?(self, currentPage)
...
//    open weak var downloadDelegate: LightboxControllerDownloadDelegate?
    open var viewModel: LightboxControllerProtocol
    ...
    public init(_ viewModel: LightboxControllerViewModel, images: [LightboxImage] = [], startIndex index: Int = 0) {
...
//        imageTouchDelegate?.lightboxController(self, didTouch: images[currentPage], at: currentPage)
        viewModel.didTouch?(self, images[currentPage], currentPage)
...
extension LightboxController: HeaderViewDelegate, HeaderViewDownloadDelegate {
    
    func headerView(_ headerView: HeaderView, didPressDownloadButton downloadButton: UIButton) {
//        downloadDelegate?.lightboxController(self, page: currentPage, didPressDownloadButton: downloadButton)
        viewModel.didPressDownloadButton?(self, currentPage, downloadButton)
    }
